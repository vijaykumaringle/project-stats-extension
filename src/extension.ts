...
interface ProjectStats {
    ...
    securityScanning: SecurityAnalysis;
    performanceHotspots: PerformanceAnalysis;
    testCoverage: TestCoverageAnalysis;
}

interface DependencyAnalysis {
    packageManagers: {
        npm: { packages: number; outdated: number; vulnerable: number };
        pip: { packages: number; outdated: number; vulnerable: number };
        cargo: { packages: number; outdated: number; vulnerable: number };
    };
...
interface TimeBasedInsights {
    ...
    mostActiveFiles: Array<{ path: string; modifications: number; lastModified: Date }>;
    inactiveFiles: number;
    developmentVelocity: 'low' | 'medium' | 'high';
    avgModificationsPerFile: number;
}

interface SecurityAnalysis {
    hardcodedSecrets: Array<{ file: string; type: string; line: number }>;
    vulnerabilityPatterns: Array<{ file: string; pattern: string; severity: number }>;
    insecurePractices: Array<{ file: string; practice: string; recommendation: string }>;
    riskLevel: 'low' | 'medium' | 'high';
}

interface PerformanceAnalysis {
    largeFunctions: Array<{ file: string; functionName: string; lines: number; complexity: number }>;
    nestedLoops: Array<{ file: string; line: number; depth: number }>;
    blockingOperations: Array<{ file: string; type: string; line: number }>;
...
async function calculateProjectStats(rootPath: string): Promise<ProjectStats> {
    ...
    const fileModifications = new Map<string, { modifications: number; lastModified: Date }>();
...
async function getFileModificationStats(filePath: string): Promise<{ modifications: number; lastModified: Date }> {
    try {
        const fs = await import('fs');
...
async function analyzeDependencies(rootPath: string): Promise<DependencyAnalysis> {
...
async function analyzeTimeBasedInsights(rootPath: string, fileModifications: Map<string, any>): Promise<TimeBasedInsights> {
    const insights: TimeBasedInsights = {
        projectAgeDays: 0,
        mostActiveFiles: [],
        inactiveFiles: 0,
        developmentVelocity: 'medium',
...
async function performSecurityScan(rootPath: string): Promise<SecurityAnalysis> {
...
async function analyzePerformanceHotspots(rootPath: string): Promise<PerformanceAnalysis> {
...
async function analyzeTestCoverage(rootPath: string): Promise<TestCoverageAnalysis> {
...
Most Active Files:
${stats.timeBasedInsights.mostActiveFiles.slice(0, 3).map((file, index) => 
    `${(index + 1)}. ${file.path.split('/').pop()} - ${file.modifications} modifications`
).join('\n  ')}
...
${stats.securityScanning.hardcodedSecrets.slice(0, 3).map((secret, index) => 
    `${index + 1}. ${secret.type} in ${secret.file}:${secret.line}`
).join('\n  ')}
...
${stats.performanceHotspots.largeFunctions.slice(0, 3).map((func, index) => 
    `${index + 1}. ${func.functionName} in ${func.file.split('/').pop()} (${func.lines} lines, complexity: ${func.complexity})`
).join('\n  ')}
...
${stats.largestFiles.map((file, index) => 
    `${(index + 1).toString().padStart(2)}. ${file.path.split('/').pop()?.padEnd(25)} ${file.lines.toString().padStart(6)} lines`
).join('\n')}
